...
function MiniSidebar() {
    ...
    const getStrokeColor = (link: string) => {
        return pathname === link ? "#B91C1C" : "#71717a";
    }
    ...
    return (
        <div className='basis-[5rem] flex flex-col bg-white'>
                ...
                <ul className="flex flex-col gap-10">
                    {navItems.map((item, index) => (
                        <li key={index} className="relative group">
                            ...
                            <span className=" absolute top-[50%] translate-y-[-50%] left-8 text-xs pointer-events-none text-white bg-red-800 px-2 py-1 rounded-md shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                {item.title}
                            </span>
                        </li>
